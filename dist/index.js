...
async function apiRequest(endpoint, options = {}) {
    ...
    if (!key) {
        throw new Error("HIREFLESH_API_KEY is not set. " +
...
function T_String(opts) {
    return { type: "string", ...opts };
}
function T_Number(opts) {
    return { type: "number", ...opts };
}
function T_Optional(schema) {
    return schema; // optionality is declared via the required array in T_Object
}
function T_Object(props, required) {
    return {
        type: "object",
        properties: props,
...
const tools = [
    // -------------------------------------------------------------------------
    {
        ...
        description: "Post a new bounty task to the HireFlesh marketplace for human workers to bid on. " +
        ...
        parameters: T_Object({
            ...
            category: T_Enum(["Data Collection", "Content Creation", "Research", "Testing", "Verification", "Photography", "Delivery", "Other"], { description: "Task category" }),
        ...
        async execute(_id, params) {
            try {
                ...
                if (params.maxWorkers !== undefined)
                    body.maxWorkers = params.maxWorkers;
                if (params.deadlineStart)
                    body.deadlineStart = params.deadlineStart;
                if (params.deadlineEnd)
                    body.deadlineEnd = params.deadlineEnd;
                if (params.location)
                    body.location = params.location;
                if (params.requiredSkills) {
                    try {
                        body.requiredSkills = JSON.parse(params.requiredSkills);
                    }
                    catch { /**/ }
                }
    ...
    // -------------------------------------------------------------------------
    {
        ...
        parameters: T_Object({ taskId: T_String({ description: "The task ID" }) }, ["taskId"]),
        async execute(_id, params) {
            try {
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async execute(_id, params) {
            try {
                ...
                if (params.skills) {
                    try {
                        const arr = JSON.parse(params.skills);
                        arr.forEach((s) => q.push(`skills[]=${encodeURIComponent(s)}`));
                    }
                    catch { /**/ }
                }
                if (params.location)
                    q.push(`location=${encodeURIComponent(params.location)}`);
                if (params.minRating !== undefined)
                    q.push(`minRating=${params.minRating}`);
    ...
    // -------------------------------------------------------------------------
    {
        ...
        async execute(_id, params) {
            try {
                const q = [];
                if (params.taskId)
                    q.push(`taskId=${encodeURIComponent(params.taskId)}`);
                if (params.status)
    ...
    // -------------------------------------------------------------------------
    {
        ...
        description: "Generate a short pairing code the operator can enter at hireflesh.com/dashboard to link " +
    ...
    // -------------------------------------------------------------------------
    {
        ...
        description: "Check whether the operator has approved the pairing code. " +
            "If approved, returns the API key and saves it to process.env.HIREFLESH_API_KEY for this session.",
        parameters: T_Object({ pairingId: T_String({ description: "The pairing ID returned by get_pairing_code" }) }, ["pairingId"]),
        async execute(_id, params) {
            try {
                const result = await publicRequest(`/api/pairing/status/${params.pairingId}`);
                if (result && typeof result === "object" && result.apiKey) {
                    process.env.HIREFLESH_API_KEY = result.apiKey;
                }
                return text(result);
            }
            catch (e) {
